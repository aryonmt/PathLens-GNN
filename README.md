...
The notebook default is `resource_allocation` / `STAGE=eval`. Push the branch before the kernel clones GitHub:

```bash
python -m pathlens run --method resource_allocation --stage eval --device cuda:0
```
