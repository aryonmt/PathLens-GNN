...
The notebook default is `skipgnn` / `STAGE=eval`. Push the branch before the kernel clones GitHub:

```bash
python -m pathlens run --method skipgnn --stage eval --device cuda:0
```
