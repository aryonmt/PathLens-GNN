...
#
# Same repaired BCE loop as official SkipGNN, on the original graph only.
...
def run_gcn(
    ...
    *,
    ...
    original, _skip = build_skipgnn_adjacencies(split.num_drugs, split.num_proteins, visible)
    num_entities = split.num_drugs + split.num_proteins
    features = _sparse_identity(torch, num_entities, device)
    adj = _to_torch_sparse(torch, original, device)

    from pathlens.model.decode import score_from_embeddings
    from pathlens.model.gcn import GCN
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            f"[gcn] epoch={epoch}/{epochs} loss={last_loss:.4f} "
