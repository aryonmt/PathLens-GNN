...
from pathlens.data.canonical import load_biosnap_tsv
from pathlens.data.negative import sample_degree_matched_negatives, sample_uniform_negatives
from pathlens.data.schema import CanonicalDataset, Edge
...
def prepare_biosnap_dataset(
    ...
    *,
    ...
    if strict_identity:
        _assert_canonical_identity(dataset)
    ...
    (output / "entities.json").write_text(
        json.dumps({"drugs": list(dataset.drugs), "proteins": list(dataset.proteins)}, indent=2),
        encoding="utf-8",
    )
    ...
    manifest: dict[str, object] = {
        ...
        "seed": seed,
        "source": {"path_hint": Path(source).name, "sha256": dataset.source_sha256},
        "counts": {
...
def _assert_canonical_identity(dataset: CanonicalDataset) -> None:
    if dataset.source_sha256 != EXPECTED_SOURCE_SHA256:
        raise ValueError(
            "Source SHA-256 does not match the locked BioSNAP file in docs/research/DATA.md"
        )
    if (
        len(dataset.edges) != EXPECTED_EDGES
        or len(dataset.drugs) != EXPECTED_DRUGS
        or len(dataset.proteins) != EXPECTED_PROTEINS
        or dataset.entity_count != EXPECTED_ENTITIES
    ):
        raise ValueError("Canonical edge/entity counts do not match the locked BioSNAP identity")
