...
No post-fusion MLP is allowed because it would make exported channel-logit contributions unfaithful.
...
- Every explanation carries truncation and provenance metadata.
...
Research evaluation uses sealed splits. After the architecture is frozen, a separately labelled product artifact may retrain the locked configuration using all curated positives for candidate ranking. It must never be reported as a test result, and all known positives remain filtered from recommendations.
